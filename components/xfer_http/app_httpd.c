...
#include "sdkconfig.h"
#include "esp_camera.h"  // 确保包含了摄像头接口头文件
...
void app_httpd_main()
{
    ...
    if (httpd_start(&camera_httpd, &config) == ESP_OK) {
        ...
        httpd_register_uri_handler(camera_httpd, &stream_uri);  // ? 把 stream 注册在 camera_httpd
    }

    // ? 不再启动第二个 server，不需要 stream_httpd
}

#define DEMO_UVC_XFER_BUFFER_SIZE (128 * 1024)  // 增加缓冲区
