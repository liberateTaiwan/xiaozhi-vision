...
class AsyncPerformanceTester:
    def __init__(self, test_type="all"):
        """
        初始化性能测试器
        :param test_type: 测试类型，可选值："all"（默认，测试所有）, "llm"（仅测试大模型）, "tts"（仅测试TTS）
        """
        self.config = read_config(get_config_file())
        self.test_type = test_type.lower()
        if self.test_type not in ["all", "llm", "tts"]:
            raise ValueError("test_type必须是'all'、'llm'或'tts'之一")
    ...
    def _print_results(self):
        ...
        # 只有在测试所有模块时才显示组合建议
        if self.test_type == "all" and self.results["combinations"]:
            print("\n推荐配置组合 (得分越小越好):")
            combo_table = []
            for combo in self.results["combinations"][:10]:
                combo_table.append([
    ...
    async def run(self):
        ...
        # 打印找到的可用模块数量
        llm_count = len([t for t in all_tasks if 'test_single_sentence' in str(t)]) / len(self.test_sentences) if self.test_type in ["all", "llm"] else 0
        tts_count = len([t for t in all_tasks if '_test_tts' in str(t)]) if self.test_type in ["all", "tts"] else 0
        
        if self.test_type in ["all", "llm"]:
            print(f"\n✅ 找到 {llm_count:.0f} 个可用LLM模块")
        if self.test_type in ["all", "tts"]:
            print(f"✅ 找到 {tts_count} 个可用TTS模块")
        print("\n⏳ 开始并发测试所有模块...\n")
...
async def main():
    ...
    if test_type not in ["all", "llm", "tts"]:
        print(f"⚠️ 无效的测试类型: {test_type}，将使用默认值'all'")
        test_type = "all"
    
    # 创建测试器实例并运行
    tester = AsyncPerformanceTester(test_type=test_type)
    await tester.run()
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        print("使用说明：")
        print("设置环境变量 TEST_TYPE 来选择测试类型：")
        print("  export TEST_TYPE=all  # 测试所有模块（默认）")
        print("  export TEST_TYPE=llm  # 仅测试大模型")
        print("  export TEST_TYPE=tts  # 仅测试TTS")
        print("\n示例：")
        print("  TEST_TYPE=llm python performance_tester.py")
        sys.exit(1)
    
    asyncio.run(main())

#TEST_TYPE=tts python performance_tester.py
#TEST_TYPE=llm python performance_tester.py