...
from core.handle.wakeWordHandler import WakeWordHandler
from core.providers.voice.ali_voiceprint import AliVoiceprintRecognition
TAG = __name__
...
class ConnectionHandler:
    def __init__(self, config: Dict[str, Any], _vad, _asr, _llm, _tts, _music):
        ...
        # 唤醒词处理器
        self.wake_word_handler = WakeWordHandler(config)

        # 初始化声纹识别模块
        voiceprint_config = config.get("voiceprint", {})
        if voiceprint_config.get("enabled", False):
            if voiceprint_config.get("provider") == "alibaba":
                self.voiceprint = AliVoiceprintRecognition(voiceprint_config)
            else:
                # 使用默认的或其他声纹识别提供商
    ...
    def chat(self, query, image_path=None):
        ...
        try:
            ...
            # 如果有图像路径，记录日志
            if image_path:
                self.logger.bind(tag=TAG).info(f"使用多模态模型处理请求，图片路径: {image_path}")
            
            # 支持多模态模型调用
            llm_responses = self.llm.response(self.session_id, self.dialogue.get_llm_dialogue(), image_path)
        except Exception as e:
    ...
    def recode_first_last_text(self, text):
        if not self.tts_first_text:
            self.logger.bind(tag=TAG).info(f"第一句: {text}")
